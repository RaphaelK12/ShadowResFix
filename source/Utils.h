...
#pragma comment( lib, "../source/minhook/build/VC22/lib/MinHook.lib" )
...
namespace Utils {
    ...
    static bool GetGameVersion(int32_t& gameVersion) {
        ...
        if(fileInfoSize) {
            ...
            if(GetFileVersionInfo("GTAIV.exe", NULL, fileInfoSize, versionInfo)) {
                UINT versionSize = 0;
                LPBYTE versionData = NULL;

                if(VerQueryValue(versionInfo, "\\", (void**) &versionData, &versionSize)) {
                    if(versionSize) {
